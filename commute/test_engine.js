...
test('Strictly dominated alternatives are marked, not removed or called cancelled',()=>{
 const rows=[{id:'a',journeyStart:100,journeyArrivalLatest:500},{id:'b',journeyStart:120,journeyArrivalLatest:480},{id:'equal',journeyStart:90,journeyArrivalLatest:480}];
...
test('Punctuality distinguishes unknown, on-time, minor/major delay and early',()=>{
 for(const [delta,tone] of [[0,'on-time'],[179,'minor-delay'],[180,'major-delay'],[-60,'early']])assert.equal(E.timingStatus({quality:'live',departure:1000+delta,scheduledDeparture:1000}).tone,tone);
 ...
 assert.equal(E.timingStatus({quality:'live',arrival:1200,scheduledArrival:1000},'arrival').tone,'major-delay');
});
...
test('Shuttle can be mathematically dominated but must never be collapsed',()=>{
 const rows=[{id:'sh',operator:'shuttle',journeyStart:100,journeyArrivalLatest:500},{id:'bus',operator:'stib',journeyStart:120,journeyArrivalLatest:480}];
